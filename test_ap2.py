...
def sequence_data(X,time_steps):
    ...
    for i in range(int(np.shape(X)[0]/time_steps)):
        print(i)
        X_current = X[i*time_steps : (i+1)*time_steps, ]
...
inputs = tf.placeholder(tf.float32, [None, backprop_length ,input_size])
...
cost = tf.reduce_sum(tf.pow(prediction - outputs, 2)/(2*training_size))
optimizer = tf.train.AdamOptimizer(learning_rate = learning_rate)
minimize = optimizer.minimize(cost)
error_test = tf.reduce_sum(tf.pow(prediction - outputs,2)/(2*test_size))
...
for epoch_idx in range(n_epochs):
    ...
    for batch_idx in range(num_batches):
        ...
        _current_state,_ = sess.run([current_state,minimize], feed_dict = {inputs: x, outputs: y,
